...
BOOL QueryDrive(char *pStr) {
  ...
  int i = 0;
	...
	sprintf(mTest, "%s\\%s", mDrive, SMTP_SERVICE);
	strcat(mDrive, "\\*");
...
DWORD KeyFileDeleteKey(char *pKeyRoot, char *pKey)
{
  char mKeyName[512], mFn[512];
  HANDLE             hF;
  WIN32_FIND_DATA    FD;
  BOOL bFile = TRUE;
#ifdef UPDATE_20070521 // 予約語対策
  char *pdom, *p, *q, mKey[512];
  BOOL bReservedWords = FALSE;
...
#ifdef UPDATE_20070521 // 予約語対策
         if (bReservedWords)
		   sprintf(mFn, "%s\\reg\\%s\\%s", pKeyRoot, mKey, FD.cFileName);
		 else
#endif
           sprintf(mFn, "%s\\reg\\%s\\%s", pKeyRoot, pKey, FD.cFileName);
         DeleteFile(mFn);
         _rmdir(mFn);
	   }
       bFile = FindNextFile( hF, &FD);
