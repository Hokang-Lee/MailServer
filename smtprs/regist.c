...
#else
#ifdef K_SEARCH // K_SEARCH OEM 版 //レジストコード情報 2007-0326-1613#%10%#
  char mKeyReg[14][9] = {"2pinSCAN","0At Peak","0NY AgE ","7*-+;XQ;","-uN PiCA","0oDeouch","3ITip.l.","2AnD3.25","6&&@@$%Q","-aNgKiBE","1nUNiDEM","6+**-OK-","1iT yegg","3T  MRI "};
#else
...
void Regist(char *regdata)
{
  ...
  WriteProfileStringEx(SOFT_REG, LIMITKEY, regdata);
#else
...
BOOL RegistUser(char *pmode, BOOL *bPassport)
{
  ...
  //// ライセンスチェックをしないと処理速度が向上する？
  // strcpy(pmode, "Bld.Keitec");
  // *bPassport = TRUE;
  // return bkey;
  ///////////////////////////////////////
...
#endif
#ifdef K_SEARCH // K_SEARCH OEM 版 //KTEC SMTP Archive Receiver
  GetProfileStringEx(SOFT_REG, LIMITKEY, "", mInpReg, sizeof(mInpReg));
#else
...
BOOL LDAP_Option_Check(void)
{
  ...
  { // 解除キー不一致なら
    ...
    if(ltn > ltb) 
	{
	  ...
	  AddToMessageLog(TEXT("Exceeded an expiration date. (LDAP Option)"), 101, TEXT("EPSTRS"), EVENTLOG_WARNING_TYPE );
	  //bSts = FALSE;
	} else {
...
#endif
{
  ...
  if (*ltbegin == 0) {
	...
	*ltbegin = u1->QuadPart;
#ifdef K_SEARCH // K_SEARCH OEM 版 //KTEC SMTP Archive Receiver
    WriteProfileIntEx(SOFT_REG, BEGINHIGH, (DWORD)ltime.dwHighDateTime);
    WriteProfileIntEx(SOFT_REG, BEGINLOW, (DWORD)ltime.dwLowDateTime);
#else
  ...
  if (!RegistUser(mode, bPassport)) {
    ...
    *bPassport ^= ReSetTimeLimit(ltbegin);
    ...
    if(lt > ltb) {
	   ...
	   AddToMessageLog(TEXT("Exceeded an expiration date."), 101, TEXT("EPSTRS"), EVENTLOG_WARNING_TYPE );
#else
	   if (bDebug) printf("Exceeded an expiration date, service was stopped.\n"); // 使用期限メッセージ
	   AddToMessageLog(TEXT("Exceeded an expiration date, service was stopped."), 101, TEXT("SPARS stop."), EVENTLOG_WARNING_TYPE );
